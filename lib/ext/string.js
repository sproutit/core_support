...
/**
  ...
  Additional useful string operators
*/
...
var STRING_TITLEIZE_REGEXP = (/([\s|\-|\_|\n])([^\s|\-|\_|\n]?)/g),
    ...
    STRING_HUMANIZE_REGEXP = (/[\-_]/g),
    STRING_TRIM_REGEXP = (/^\s+|\s+$/g),
    STRING_TRIM_LEFT_REGEXP = (/^\s+/g),
    STRING_TRIM_RIGHT_REGEXP = (/\s+$/g),
    STRING_REGEXP_ESCAPED_REGEXP = (/([\\\.\+\*\?\[\^\]\$\(\)\{\}\=\!\<\>\|\:])/g),
    DIACRITIC_MAPPING_TABLE;
    

Cs.mixin(String.prototype, {
  ...
  */
  capitalizeEach: function() {
    return this.replace(STRING_TITLEIZE_REGEXP, 
  ...
  */
  titleize: function() {
    var ret = this.replace(STRING_DECAMELIZE_REGEXP,'$1_$2'); // decamelize
  ...
  */
  camelize: function() {
    var ret = this.replace(STRING_TITLEIZE_REGEXP, 
      function(str,separater,character) { 
        return (character) ? character.toUpperCase() : '' ;
      }) ;
  ...
  */
  decamelize: function() { 
    return this.replace(STRING_DECAMELIZE_REGEXP,'$1_$2').toLowerCase();
  },
  ...
  */
  dasherize: function() {
    return this.decamelize().replace(STRING_DASHERIZE_REGEXP,'-') ;  
  },
  ...
  */
  humanize: function() {
    return this.decamelize().replace(STRING_HUMANIZE_REGEXP,' ') ;
  },
  ...
  */
  escapeForRegExp: function() {
    return this.replace(STRING_REGEXP_ESCAPED_REGEXP, "\\$1");
  },
  ...
  */
  removeDiacritics: function() {
    ...
    // Lazily create the mapping table
    if (!DIACRITIC_MAPPING_TABLE) {
      DIACRITIC_MAPPING_TABLE = {
    ...
    for (var i = 0; i <= length; ++i) {
      ...
      replacement = DIACRITIC_MAPPING_TABLE[original];
      if (replacement) ret += replacement;
      else ret += original;
    }
    return ret;
  },
...
/** @private */
String.prototype.strip = String.prototype.trim; // convenience alias.
