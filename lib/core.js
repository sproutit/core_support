...
function _Cextend() {
  ...
  args.unshift(this);
  return Cs.extend.apply(Cs, args);
}
...
/**
  ...
  | Cs.T_STRING | String primitive |
  | Cs.T_NUMBER | Number primitive |
  | Cs.T_BOOLEAN | Boolean primitive |
  | Cs.T_NULL | Null value |
  | Cs.T_UNDEFINED | Undefined value |
  | Cs.T_FUNCTION | A function |
  | Cs.T_ARRAY | An instance of Array |
  | Cs.T_CLASS | A SproutCore class (created using Cs.Object.extend()) |
  | Cs.T_OBJECT | A SproutCore object instance |
  | Cs.T_HASH | A JavaScript object not inheriting from Cs.Object |

  @param item {Object} the item to check
  @returns {String} the type
*/  
Cs.typeOf = function(item) {
  if (item === undefined) return Cs.T_UNDEFINED ;
  if (item === null) return Cs.T_NULL ; 
  var ret = typeof(item) ;
  if (ret == "object") {
    if (item instanceof Array) {
      ret = Cs.T_ARRAY ;
    } else if (item instanceof Function) {
      ret = item.isClass ? Cs.T_CLASS : Cs.T_FUNCTION ;

    // NB: typeOf() may be called before Cs.Error has had a chance to load
    // so this code checks for the presence of Cs.Error first just to make
    // sure.  No error instance can exist before the class loads anyway so
    // this is safe.
    } else if (Cs.Error && (item instanceof Cs.Error)) {
      ret = Cs.T_ERROR ;        
    } else if (item.isObject === true) {
      ret = Cs.T_OBJECT ;
    } else ret = Cs.T_HASH ;
  } else if (ret === Cs.T_FUNCTION) ret = (item.isClass) ? Cs.T_CLASS : Cs.T_FUNCTION;
  return ret ;
};
...
/**
  ...
  Cs.Object-based or not, but be aware that it will add a _guid property.
...
/**
  ...
  call Cs.guidFor(obj), which return an existing guid if available.
...
/**
 ...
 The order is calculated based on Cs.ORDER_DEFINITION , if types are different.
...
Cs.compare = function (v, w) {
  ...
  // If we haven't yet generated a reverse-mapping of Cs.ORDER_DEFINITION,
  // do so now.
  ...
  if (!mapping) {
    ...
    // We no longer need Cs.ORDER_DEFINITION.
