...
var PATH = require('default:path'),
    ...
    CHILD_PROCESS = require('default:child_process'),
...
fs.exec = function(command, done) {
  if (done === undefined) {
    return CS_ITER.wait(function(done) { CHILD_PROCESS.exec(command, done);});
  } else CHILD_PROCESS.exec(command, done);
};
...
fs.mkdir_p = function(path, mode, done) {
  if (done === undefined) {
    if (!fs.exists(path)) return FS.mkdirSync(path, mode);
    else return false;
  } else {
    fs.exists(path, function(err, exists) {
      if (err) return done(err);
      if (!exists) return FS.mkdir(path, mode, done);
      return done();
    });
  }
};
