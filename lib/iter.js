...
/**
  ...
  {{{
    Cs.map(someItems, function(item, done) {
      // make some changes
      return done(null, changedItem); 
    })(function(err, items) {
      Cs.reduce(0, items, function(val, item, done) {
        done(null, val + item);
      });
    })(function(err, count) {
  ...
  {{{
    var mapIt = Cs.map(someItems, function(item, done) {
      // make some changes
      return done(null, changedItem);
    });
    ...
    var countIt = Cs.chain(mapIt, reduceIt);
...
/**
  ...
  {{{
    ...
    var loadit = Cs.once(Cs.fs.loadFile(pathToFile));
  ...
  @returns {Function} 
    ...
    result.
*/
Cs.once = function(action, context) {
  var state = 'pending',
      queue = [],
      args  = null;
...
Cs.wait = function(cont, context) {
  ...
  args[args.length] = function(err, val) {
    ...
    unlooped = true;
    if (looped) require.seed.unloop();
  };
  cont.apply(context, args);

  looped = true;
  if (!unlooped) require.seed.loop(); // wait until finished
  return ret;
};
...
/**
  ...
  synchronously.
*/
Cs.sync = function(cont, context) {
  return function() {
    var args = Array.prototype.slice(arguments);
    args.unshift(cont);
    return Cs.wait.apply(Cs, args, context);
  };
};
...
*/
Cs.async = function(func, context) {
  return function() {
    var args = Array.prototype.slice(arguments),
...
*/
Cs.reduce = function(array, initial, fn) {
  var len = array.length;
  return function(done) {
    var idx = -1,
        ret = initial;
...
/**
  ...
  Unlike Cs.map() and Cs.each() you must pass an array and function.
  
*/
Cs.filter = function(array, fn) {
  var len = array.length;
  return function(done) {
    var idx = -1,
        ret = [];
...
/**
  Just like Cs.each() except each item in invoked in parallel with the 
  callback invoked when all items have returned or when one returns with 
  an error. 
  
  Just like Cs.each() you can pass an array and function to invoke on each
...
/**
  Works just like Cs.filter() except that each item is dispatched in parallel
  instead of in series.  Use this form when you don't care what order the
...
*/
Cs.filterParallel = function(array, fn) {
  var len ;
  ...
  return function(done) {
    ...
    // see perf note in Cs.collect() - similar problem here
    var tail = function(idx) {
      return function(err, val) {
        if (cancelled) return; // nothing to do
